...
export async function Impl(
  ...
  try {
              ...
              ContainerOverrides: [
                {
                  ...
                  Environment: [
                    ...
                    { Name: "REPORT_DATA_BUCKET", "Value.$": "$.payload.report_bucket" },
                    { Name: "TASK_TOKEN_ENV_VARIABLE", "Value.$":"$$.Task.Token" }
                  ],
                },
              ],
            },
    ...
    await asyncForEach(records, async (record: Record) => {
      ...
      record.payload.taskDef = (scanTaskArns as {[index: string]: string})[record.payload.name];
      record.payload.reportBucket = (scanReportBuckets as {[index: string]: string})[record.payload.name];
